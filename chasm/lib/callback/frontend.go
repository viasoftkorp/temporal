...
// > Roey: Make sure to validate all identifiers (e.g. callback ID) and any other field with dynamic length (string, list, map).
// > You can follow what @stephanos did in the standalone nexus branch.
// > ...
// > Also validate search attributes here.
func validateStartCallbackExecutionRequest(request *workflowservice.StartCallbackExecutionRequest, config *Config) error {
	...
	// TODO(chrsmith): Unresolved comment. "Validate payload size here."
	if completion == nil {
...
func (h *frontendHandler) StartCallbackExecution(
	...
	if !h.config.EnableStandaloneExecutions(request.GetNamespace()) {
		return nil, ErrStandaloneCallbackDisabled
	}
	// TODO(chrsmith): Same as elsewhere, validate callback IDs.
...
func (h *frontendHandler) DescribeCallbackExecution(
	...
	// TODO(chrsmith): Unresolved comment. "Validate the ID, run ID, and long poll token"
	if request.GetCallbackId() == "" {
		return nil, serviceerror.NewInvalidArgument("CallbackId is not set on request.")
	}
...
func (h *frontendHandler) PollCallbackExecution(
	...
	// TODO(chrsmith): Unresovled comment. "Same as above, more field validation required."
	if request.GetCallbackId() == "" {
		return nil, serviceerror.NewInvalidArgument("CallbackId is not set on request.")
	}
