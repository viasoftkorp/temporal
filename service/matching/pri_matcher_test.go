...
func (s *PriMatcherSuite) TestValidatorDrop_SetsDropReason() {
	cases := []struct {
		...
		wantReason metrics.Tag
	}{
		{"ExpiredMemory", timestamppb.New(time.Now().Add(-time.Hour)), metrics.DroppedTaskReasonExpiredMemoryTag},
		{"Invalid", nil, metrics.DroppedTaskReasonInvalidTag},
	}
	...
	for _, tc := range cases {
		s.Run(tc.name, func() {
			...
			case res := <-completionCalled:
				...
				s.Require().NotNil(res.dropReason)
