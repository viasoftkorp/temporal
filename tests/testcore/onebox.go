...
func (c *TemporalImpl) serverOptionsForHost(
	...
	options := []temporal.ServerOption{
		...
		temporal.WithTestHooks(c.testHooks),
...
func (c *TemporalImpl) installHostTestHooks(
	...
	addCleanup(testhooks.Set(
		...
		func(registry any) error {
			return registry.(*chasm.Registry).Register(chasmtests.Library)
		},
		testhooks.GlobalScope,
	))
	addCleanup(testhooks.Set(
	...
	case primitives.FrontendService:
		addCleanup(testhooks.Set(
			c.testHooks,
			testhooks.FrontendServiceRefsCreated,
			func(refs testhooks.FrontendServiceRefs) {
				if c.frontendClient != nil {
					return
				}

				// This connection/clients uses membership to find frontends and load-balance among them.
				rpcFactory := refs.RPCFactory.(common.RPCFactory)
				...
				c.historyClient = refs.HistoryClient.(historyservice.HistoryServiceClient)
				c.matchingClient = refs.MatchingClient.(matchingservice.MatchingServiceClient)
				c.schedulerClient = refs.SchedulerClient.(schedulerpb.SchedulerServiceClient)
			},
			testhooks.GlobalScope,
		))
	case primitives.HistoryService:
		addCleanup(testhooks.Set(
			c.testHooks,
			testhooks.HistoryServiceRefsCreated,
			func(refs testhooks.HistoryServiceRefs) {
				c.chasmEngine = refs.ChasmEngine.(chasm.Engine)
				c.chasmVisibilityMgr = refs.ChasmVisibilityManager.(chasm.VisibilityManager)
				c.chasmRegistry = refs.ChasmRegistry.(*chasm.Registry)
			},
			testhooks.GlobalScope,
		))
		addCleanup(testhooks.Set(
			...
			func(base any, logger log.Logger) any {
				...
				c.taskQueueRecorder = NewTaskQueueRecorder(base.(persistence.ExecutionManager), logger)
