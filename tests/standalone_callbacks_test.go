...
// be processed, or a notification from a datastore.)
//
// Run directly via:
/*
go test \
	...
	-test.v \
	-timeout 10s \
	-run '^TestStandaloneCallbackSuite$' \
	-testify.m '^(TestBasicaOperation)$'
...
# To combat spammy logs, use tee.
go test ./tests/ \
	-timeout 10s \
	-v \
	-run TestStandaloneCallbackSuite -count=1 2>&1 | tee test.log
*/

// externalRequestInfo captures the minimal information that an external
// service would need in order to notify Temporal that a standalone callback
// has been completed.
type externalRequestInfo struct {
	...
	// Data to be sent back to Temporal. Required.
	Result *callbackpb.CallbackExecutionCompletion
}

// externalRequestResult is the result of the faux external system
// processing a request. If accepted, both TokenID and RunID will be
// set. Otherwise they will be nil, and Error will be set instead.
type externalRequestResult struct {
	// CallbackID of the callback execution.
	CallbackID *string
	// RunID of the callback execution.
	RunID *string
	Error error
}

// fauxExternalService simulates an external service, separate from Temporal.
// It receives requests to do work via Nexus Callbacks, and then
type fauxExternalService struct {
	incommingRequests chan<- externalRequestInfo
	requestResults    <-chan externalRequestResult
}

// startFauxExternalService returns a new fauxExternalService, and spawns its
// execution in a Goroutine. Will shutdown and close channels when the
// supplied context is Done.
func startFauxExternalService(ctx context.Context, client workflowservice.WorkflowServiceClient) *fauxExternalService {
	// Channels so that tests don't actually block when reading/writing.
	input := make(chan externalRequestInfo, 2)
	output := make(chan externalRequestResult, 2)

	// Logic of the actual faux return, processing requests.
	...
	go func() {
		...
		for {
			...
			case incommingRequest := <-input:
				...
				// Report the result to the testcase.
				var result externalRequestResult
				if err == nil {
...
// allowing cascading failures. e.g. "s.Require().NotNil(err)"
...
func (s *StandaloneCallbackSuite) SetupSuite() {
	s.SetupSuiteWithCluster(
		testcore.WithDynamicConfigOverrides(map[dynamicconfig.Key]any{
			...
			// As a workaround, we specify the HSM rules.
...
// 			Variant: &commonpb.Callback_Nexus_{
// 				Nexus: &commonpb.Callback_Nexus{
// 					Url: "http://localhost:0/nonexistent",
// 				},
// 			},
// 		}
// 	}
...
// TestBasicaOperation tests that a Nexus operation started by a
...
func (s *StandaloneCallbackSuite) TestBasicOperation() {
	...
	// External Service
	//
	// Start a faux external service to handle async requests, and report
	// their result to Temporal via StartCallbackExecution.
	fauxService := startFauxExternalService(ctx, s.FrontendClient())

	// Nexus Handler
	//
	// Set up an external Nexus handler that starts operations asynchronously
	// and sends the callback URL + token to the faux external process.

	// The payload the external service sends, which we will verify is received correctly.
	...
	nexusHandler := nexustest.Handler{
		OnStartOperation: func(
			...
			// The Nexus operation will then complete.
			fauxService.incommingRequests <- externalRequestInfo{
				Namespace: s.Namespace().String(),
	...
	// Register the Nexus endpoint with the service, which the calling workflow will invoke.
	// And will ultimately be resolved via standalone callback.
	...
	// Calling Workflow
	//
	// This will create the Nexus operation, invoking the Handler workflow.
	// Which will then block until the external service marks the Nexus
	...
	// Run the Test
	//
	// Start the Worker, executing the calling Workflow.
	worker := worker.New(s.SdkClient(), taskQueue, worker.Options{})
	worker.RegisterWorkflow(callerWf)
	s.Require().NoError(worker.Start())
	defer worker.Stop()

	// Start the Calling Workflow.
	startOpts := client.StartWorkflowOptions{
		TaskQueue: taskQueue,
	}
	run, err := s.SdkClient().ExecuteWorkflow(ctx, startOpts, callerWf)
	s.Require().NoError(err)

	// Wait for Completion
	//
	// The Caller workflow will start the Nexus operation.
	// The Nexus operation will invoke our customer handler, which will
	//    then start the faux external service.
	// The external service will then call StartExternalCallback, marking
	//    the callback as complete.
	// Which will then restart the Nexus machineary, completing the calling
	//    workflow.

	// If the faux external service was properly triggered, we expect to see some sort of
	// result.
	fauxServiceResult := <-fauxService.requestResults
	s.Require().NoError(fauxServiceResult.Error)
	...
	s.NoError(run.Get(ctx, &gotPayload))
	...
	descResp, err := s.FrontendClient().DescribeCallbackExecution(ctx, &workflowservice.DescribeCallbackExecutionRequest{
		Namespace:  s.Namespace().String(),
		CallbackId: *fauxServiceResult.CallbackID,
	})
	s.Require().NoError(err)
	s.NotNil(descResp.GetInfo())
	s.Equal(*fauxServiceResult.CallbackID, descResp.GetInfo().GetCallbackId())
